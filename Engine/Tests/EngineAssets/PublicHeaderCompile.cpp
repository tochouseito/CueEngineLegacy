...
static_assert(std::is_standard_layout_v<cue::engine_assets::BuiltInMeshProvider>);
...
int main()
{
    ...
    const auto providers = cue::engine_assets::built_in_authoring_mesh_providers();
    return cube.vertices.empty() || plane.vertices.empty() || sphere.vertices.empty() || catalog.empty() ||
                   providers.empty()
               ? 1
               : 0;
}
