#include "Sha256.h"

#include <algorithm>
#include <array>
#include <bit>
#include <cstddef>
#include <cstdint>
#include <span>

namespace
{
constexpr std::array<std::uint32_t, 64U> k_roundConstants = {
    0x428a2f98U, 0x71374491U, 0xb5c0fbcfU, 0xe9b5dba5U, 0x3956c25bU, 0x59f111f1U, 0x923f82a4U, 0xab1c5ed5U,
    0xd807aa98U, 0x12835b01U, 0x243185beU, 0x550c7dc3U, 0x72be5d74U, 0x80deb1feU, 0x9bdc06a7U, 0xc19bf174U,
    0xe49b69c1U, 0xefbe4786U, 0x0fc19dc6U, 0x240ca1ccU, 0x2de92c6fU, 0x4a7484aaU, 0x5cb0a9dcU, 0x76f988daU,
    0x983e5152U, 0xa831c66dU, 0xb00327c8U, 0xbf597fc7U, 0xc6e00bf3U, 0xd5a79147U, 0x06ca6351U, 0x14292967U,
    0x27b70a85U, 0x2e1b2138U, 0x4d2c6dfcU, 0x53380d13U, 0x650a7354U, 0x766a0abbU, 0x81c2c92eU, 0x92722c85U,
    0xa2bfe8a1U, 0xa81a664bU, 0xc24b8b70U, 0xc76c51a3U, 0xd192e819U, 0xd6990624U, 0xf40e3585U, 0x106aa070U,
    0x19a4c116U, 0x1e376c08U, 0x2748774cU, 0x34b0bcb5U, 0x391c0cb3U, 0x4ed8aa4aU, 0x5b9cca4fU, 0x682e6ff3U,
    0x748f82eeU, 0x78a5636fU, 0x84c87814U, 0x8cc70208U, 0x90befffaU, 0xa4506cebU, 0xbef9a3f7U, 0xc67178f2U};

/// @brief 一つの64-byte BlockをSHA-256 Stateへ圧縮する
void compress_block(std::array<std::uint32_t, 8U> &a_state, std::span<const std::byte, 64U> a_block) noexcept
{
    std::array<std::uint32_t, 64U> words{};
    for (std::size_t index = 0U; index < 16U; ++index)
    {
        const std::size_t offset = index * 4U;
        words[index] = (static_cast<std::uint32_t>(a_block[offset]) << 24U) |
                       (static_cast<std::uint32_t>(a_block[offset + 1U]) << 16U) |
                       (static_cast<std::uint32_t>(a_block[offset + 2U]) << 8U) |
                       static_cast<std::uint32_t>(a_block[offset + 3U]);
    }
    for (std::size_t index = 16U; index < words.size(); ++index)
    {
        const std::uint32_t first =
            std::rotr(words[index - 15U], 7) ^ std::rotr(words[index - 15U], 18) ^ (words[index - 15U] >> 3U);
        const std::uint32_t second =
            std::rotr(words[index - 2U], 17) ^ std::rotr(words[index - 2U], 19) ^ (words[index - 2U] >> 10U);
        words[index] = words[index - 16U] + first + words[index - 7U] + second;
    }

    std::uint32_t a = a_state[0U];
    std::uint32_t b = a_state[1U];
    std::uint32_t c = a_state[2U];
    std::uint32_t d = a_state[3U];
    std::uint32_t e = a_state[4U];
    std::uint32_t f = a_state[5U];
    std::uint32_t g = a_state[6U];
    std::uint32_t h = a_state[7U];
    for (std::size_t index = 0U; index < words.size(); ++index)
    {
        const std::uint32_t sum1 = std::rotr(e, 6) ^ std::rotr(e, 11) ^ std::rotr(e, 25);
        const std::uint32_t choice = (e & f) ^ ((~e) & g);
        const std::uint32_t temporary1 = h + sum1 + choice + k_roundConstants[index] + words[index];
        const std::uint32_t sum0 = std::rotr(a, 2) ^ std::rotr(a, 13) ^ std::rotr(a, 22);
        const std::uint32_t majority = (a & b) ^ (a & c) ^ (b & c);
        const std::uint32_t temporary2 = sum0 + majority;
        h = g;
        g = f;
        f = e;
        e = d + temporary1;
        d = c;
        c = b;
        b = a;
        a = temporary1 + temporary2;
    }
    a_state[0U] += a;
    a_state[1U] += b;
    a_state[2U] += c;
    a_state[3U] += d;
    a_state[4U] += e;
    a_state[5U] += f;
    a_state[6U] += g;
    a_state[7U] += h;
}
} // namespace

namespace cue::distribution_private
{
Sha256Digest compute_sha256(std::span<const std::byte> a_bytes) noexcept
{
    std::array<std::uint32_t, 8U> state = {
        0x6a09e667U, 0xbb67ae85U, 0x3c6ef372U, 0xa54ff53aU, 0x510e527fU, 0x9b05688cU, 0x1f83d9abU, 0x5be0cd19U,
    };
    std::size_t offset = 0U;
    while (a_bytes.size() - offset >= 64U)
    {
        compress_block(state, std::span<const std::byte, 64U>(a_bytes.data() + offset, 64U));
        offset += 64U;
    }

    std::array<std::byte, 128U> tail{};
    const std::size_t remaining = a_bytes.size() - offset;
    std::copy_n(a_bytes.data() + offset, remaining, tail.data());
    tail[remaining] = std::byte{0x80U};
    const std::size_t tailSize = remaining < 56U ? 64U : 128U;
    const std::uint64_t bitLength = static_cast<std::uint64_t>(a_bytes.size()) * 8U;
    for (std::size_t index = 0U; index < 8U; ++index)
    {
        tail[tailSize - 1U - index] = static_cast<std::byte>(bitLength >> (index * 8U));
    }
    compress_block(state, std::span<const std::byte, 64U>(tail.data(), 64U));
    if (tailSize == 128U)
    {
        compress_block(state, std::span<const std::byte, 64U>(tail.data() + 64U, 64U));
    }

    Sha256Digest digest{};
    for (std::size_t index = 0U; index < state.size(); ++index)
    {
        digest[index * 4U] = static_cast<std::uint8_t>(state[index] >> 24U);
        digest[index * 4U + 1U] = static_cast<std::uint8_t>(state[index] >> 16U);
        digest[index * 4U + 2U] = static_cast<std::uint8_t>(state[index] >> 8U);
        digest[index * 4U + 3U] = static_cast<std::uint8_t>(state[index]);
    }
    return digest;
}

std::string sha256_text(std::string_view a_text)
{
    constexpr char hexadecimal[] = "0123456789abcdef";
    const Sha256Digest digest = compute_sha256(std::as_bytes(std::span(a_text.data(), a_text.size())));
    std::string result;
    result.reserve(digest.size() * 2U);
    for (const std::uint8_t value : digest)
    {
        result.push_back(hexadecimal[value >> 4U]);
        result.push_back(hexadecimal[value & 0x0fU]);
    }
    return result;
}
} // namespace cue::distribution_private
